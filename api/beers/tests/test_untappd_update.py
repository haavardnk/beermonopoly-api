...
def test_update_beer_correctly(mocked_responses,json_response):
    ...
    """
    beer = Beer.objects.create(vmp_id=12611502,
    ...
    assert beer.ibu == 28
    assert beer.label_url == "https://api.test.com/site/beer_logos_hd/beer-100415_64d6e_hd.jpeg"
    ...
    assert beer.verified_match == True
...
def test_stop_when_no_api_calls_remaining(mocked_responses,json_response):
    ...
    """
    Beer.objects.create(vmp_id=12611503, vmp_name="Ayinger Winterbock", active=True, untpd_id=100415)
    Beer.objects.create(vmp_id=12611504, vmp_name="Ayinger Winterbock", active=True, untpd_id=100415)
